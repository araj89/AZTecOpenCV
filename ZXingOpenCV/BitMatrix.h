...
public:
	...
	*/
	...
	bool get3(int x, int y, bool clr) const {
		int sum = 0;
		int cx, cy;
		int r = 0;

		if (_bits.at(y * _width + x) != clr)
			return (!clr);

		int sx = MAX(0, x - r);
		int ex = MIN(_width, x + r + 1);
		int sy = MAX(0, y - r);
		int ey = MIN(_height, y + r + 1);

		for (int i = sx; i < ex; i++) for (int j = sy; j < ey; j++) {
			sum += (_bits.at(i * _width + j) == clr);
		}
		if (sum > (ex - sx)* (ey - sy) / 3)
			return clr;
		return (!clr);
	}
